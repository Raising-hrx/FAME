...
# Evaluation code for EntailmentBank
# The implementation has tiny differences from official evaluation code.
...
def eval_tree(pred_tree,gold_tree,bleurt_scorer,bleurt_process_fn=None,bleurt_threshold=0.28):
    """
    pred_tree and gold_tree are in node_list format
    """
    ...
    leaves_compare_sent = {'pred': {node['id']:node['sent'] for node in pred_tree if node['id'].startswith('sent')},
                   'gold': {node['id']:node['sent'] for node in gold_tree if node['id'].startswith('sent')},}
    ...
    eval_result['leaves_compare_sent'] = leaves_compare_sent
...
def preprocess_pred_tree_task3(pred_tree, gold_tree):
    """
    rename the sent node in the pred_tree according to the gold_tree for Task3
    """
    gold_leaf_sent2id = {}
...
def eval_tree_task3(pred_tree,gold_tree,bleurt_scorer,bleurt_process_fn=None,bleurt_threshold=0.28):
    processed_pred_tree = preprocess_pred_tree_task3(pred_tree, gold_tree)
    return eval_tree(processed_pred_tree,gold_tree,bleurt_scorer,bleurt_process_fn=bleurt_process_fn,bleurt_threshold=bleurt_threshold)